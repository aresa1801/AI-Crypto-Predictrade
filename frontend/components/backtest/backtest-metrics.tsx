...
function MetricCard({ label, value, unit, icon, color = 'neutral' }: MetricCardProps) {
  ...
  const borderClasses = {
    success: 'border-accent-emerald/25 bg-accent-emerald/5',
    warning: 'border-accent-amber/25 bg-accent-amber/5',
    danger: 'border-accent-red/25 bg-accent-red/5',
    neutral: 'border-border-color/60 bg-surface-secondary/40',
  }

  return (
    <div className={`relative rounded-xl border ${borderClasses[color]} p-5 backdrop-blur-sm overflow-hidden hover:scale-[1.02] transition-transform duration-300`}>
      <div className="flex items-center justify-between mb-3">
        <p className="text-sm font-semibold text-text-secondary">{label}</p>
        {icon}
      </div>
      <div className="space-y-1">
        <p className={`text-3xl font-bold tracking-tight ${colorClasses[color]}`}>
          {value}
